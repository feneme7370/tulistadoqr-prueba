    ...
    <div class="flex flex-wrap justify-center items-center py-10 bg-primary-200 gap-2">
      <div v-for="level in levelsDates" :key="level.id" >
        ...
        <div v-if="categoriesDates.some(category => category.level_id === level.id)" >
          ...
          <div @click="levelIdSelected(level.id)" class="my-2 bg-center bg-no-repeat w-28 h-28 px-1 relative">
        ...
        <div v-for="category in categoriesDates" :key="category.id">
          <div v-if="productsDates.some(product => product.category_id === category.id)">
            ...
            <div 
              ...
              class="mb-10 bg-center bg-no-repeat w-16 h-16 sm:w-20 sm:h-20 cursor-pointer relative "
